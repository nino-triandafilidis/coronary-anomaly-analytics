...
Steps:
  ...
  6. split into confirmed_AAOCA (RCA / LCA, no review flag) vs needs_medical_review (flagged)

Output (see README): JSONL (parser input) + manifest CSV (counts / QA). confirmed_AAOCA is written;
needs_medical_review is computed but not saved by default (write call commented out below).
...
def write(rows, folder, with_flags):
    os.makedirs(folder, exist_ok=True)
    with open(folder + '/reports.jsonl', 'w') as jf, open(folder + '/manifest.csv', 'w', newline='') as cf:
        cols = ['patient_id', 'date', 'side', 'age', 'title'] + (['flag', 'flag_reason'] if with_flags else [])
...
def main(export_path, out_dir):
    ...
    for pid, ss in bypat.items():
        ...
        if len(ss) > 1:
            multi += 1
            latest = ss[-1]  # edge case: latest flagged but an earlier study was clean
            if (latest['flag'] or latest['lat'] not in ('RCA', 'LEFT')) and \
               any((not s['flag'] and s['lat'] in ('RCA', 'LEFT')) for s in ss[:-1]):
                lost_clean += 1
        chosen.append(ss[-1])

    # 6. split confirmed_AAOCA (clean single-sided) vs needs_medical_review (flagged)
    confirmed = [s for s in chosen if not s['flag'] and s['lat'] in ('RCA', 'LEFT')]
    review = [s for s in chosen if s['flag'] or s['lat'] not in ('RCA', 'LEFT')]

    write(confirmed, out_dir + '/confirmed_AAOCA', False)
    # needs_medical_review is computed but NOT saved by default (flagged cases held for clinician
    # adjudication). Uncomment to write it (adds flag / flag_reason manifest columns):
    # write(review, out_dir + '/needs_medical_review', True)
    ...
    print(f"  latest flagged but had earlier clean study: {lost_clean}")
    print(f"\nCONFIRMED_AAOCA (parse-ready): {len(confirmed)} patients  {dict(Counter(SIDE.get(s['lat'], s['lat']) for s in confirmed))}")
    print(f"NEEDS_MEDICAL_REVIEW (computed, not saved): {len(review)} patients  {dict(Counter(s['flag'] for s in review))}")
    print(f"\nwrote {out_dir}/confirmed_AAOCA/  (needs_medical_review computed but not saved; see README)")
