 ...
 *   1. paper-tracked feature (bounded PAPER_FEATURES dictionary, populated by the
 *      #66 LLM entity-linking resolver via paperFeatureId, with the legacy alias
 *      tiers as fallback),
 *   2. lowercased normalized name (so case/whitespace variants still collapse).
 * Returns null only when there is no usable name.
 *
 * The generic per-vessel coronary-narrowing tier was retired in #88: it invented
 * a "<severity> <location> narrowing of <vessel>" pseudo-feature space outside the
 * bounded paper set, and its rows were dominated by negated normal-patency
 * statements. In-scope narrowing now routes to the paper entities (csa_narrowing,
 * dynamic_narrowing, effective_lumen_diameter_narrowing,
 * coronary_atherosclerotic_lesions) through the resolver instead.
 */
